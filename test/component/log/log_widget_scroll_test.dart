...
/// 中文注释：覆盖多种字符集与日志级别的真实行形态。普通日志行高度必须一致，
/// 只有 GUI Rule 分隔行可以额外留白。
...
/// 中文注释：验证 ListView 用可变固定高度保留长列表定位能力，同时只给分隔行
/// 增加 8px（上下各 4px）留白；普通行的间距不可被这一视觉调整带大。
Future<void> _expectDividerExtent(WidgetTester tester) async {
  final controller = _FakeLogController();
  controller.logs.addAll(_rowSamples);
  await _pumpLogWidget(tester, controller);

  final listView = tester.widget<ListView>(find.byType(ListView));
  final itemExtentBuilder = listView.itemExtentBuilder;
  expect(itemExtentBuilder, isNotNull);
  ...
  final normalExtent = itemExtentBuilder!(0, dimensions)!;
  final dividerExtent = itemExtentBuilder(6, dimensions)!;

  expect(dividerExtent, normalExtent + 8, reason: '分隔行应只增加上下各 4px 的留白');
  for (final index in [1, 2, 3, 4, 5, 7]) {
    expect(itemExtentBuilder(index, dimensions), normalExtent,
        reason: '普通日志行不得因分隔行留白而变高：${_rowSamples[index].trim()}');
  }
}

void main() {
  // 中文注释：锁定可变固定高度列表。它让 Sliver 能按 index 计算 offset，
  // 又允许分隔行拥有独立留白，不会退回需要逐行测量的普通 SliverList。
  testWidgets('log list declares an item extent builder', (tester) async {
    final controller = _FakeLogController();
  ...
  testWidgets('divider extent is larger in landscape', (tester) async {
    await _expectDividerExtent(tester);
  });

  testWidgets('divider extent is larger in portrait', (tester) async {
